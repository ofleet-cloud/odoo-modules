...
class S3Attachment(models.Model):
    ...
    @api.model
    def _file_read_s3_or_cache(self, fname, bin_size=False):
        ...
        if cache_domain:
            try:
                ...
                _logger.error(e)
                return self._file_read_s3(fname=fname, bin_size=bin_size)
        else:
            return self._file_read_s3(fname=fname, bin_size=bin_size)
        ...
        if s3_active:
            ...
            if disk_first:
                ...
                try:
                    r = ''
                    if bin_size:
                        r = tools.human_size(os.path.getsize(full_path))
                    else:
                    ...
                    if e.errno == errno.ENOENT:
                        ...
                        return self._file_read_s3_or_cache(fname=fname, bin_size=bin_size)
    ...
    @api.model
    def _file_write_s3(self, value, checksum):
        storage = self._storage()
        try:
            s3_bucket = s3_globals.get_s3_bucket()
    ...
    def _file_write(self, value, checksum):
        ...
        if s3_active:
            ...
            if disk_first:
                ...
                return self._file_write_s3(value, checksum)
        else:
            return super(S3Attachment, self)._file_write(value, checksum)
    ...
    def _copy_filestore_to_s3(self):
        try:
            ...
            _logger.debug('S3: filestore copied to S3 successfully')
        except Exception:
            _logger.error("S3: filestore copy to S3 aborted!")
        return {}
